...
export class Modals {
  static open<T>(modal: React.ComponentType<T>, props: OmitType<T, Omit<UseModalProps, "onSuccess">>) {
...
export const useModal = <RequiredProps, Props extends Partial<Omit<RequiredProps, "close">>>(
  modal: React.ComponentType<RequiredProps>,
  initProps: Partial<Props> = {}
) => {
  return useCallback(Modals.prepare(modal, initProps), [modal, ...Object.keys(initProps), ...Object.values(initProps)]);
};
